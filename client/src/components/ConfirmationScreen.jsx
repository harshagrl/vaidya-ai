...
export default function ConfirmationScreen({ draftData, type, sourceImageUrl, onSaveSuccess, onCancel }) {
  ...
  const renderInput = (value, onChange, placeholder = '') => {
    ...
    return (
      <input
        ...
        className={`w-full p-2 border rounded text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none transition-colors ${
          needsReview ? 'border-red-400 bg-red-50 placeholder-red-400 focus:border-red-500 focus:ring-red-500' : 'border-gray-300 bg-white'
        }`}
      />
    );
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await api.post(`/api/profiles/${targetProfileId}/records`, payload);
